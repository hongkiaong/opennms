            ...
            for (final State rpc : State.values()) {
                for (final long heartbeatDate : dateCombinations) {
                    for (final long rpcDate : dateCombinations) {
                        final MinionStatus heartbeatStatus = new MinionServiceStatus(new Date(heartbeatDate), heartbeat);
                        final MinionStatus rpcStatus = new MinionServiceStatus(new Date(rpcDate), rpc);
